...
void mySAD_Disparity7x7(const cv::Mat &left_image, const cv::Mat &right_image, cv::Mat &out)
{
  ...
  for(int r = h; r <  left_image.rows - h; ++r)
  {
    std::cout << r << std::endl;
    for(int c = w; c <  left_image.cols - w; ++c)
    {
      float disp=0; //disparita'
		    //setto i valori dei minimi in modo che abbiano valori elevati
      uint32_t min_sum_first = std::numeric_limits<uint32_t>::max();
      uint32_t min_sum_2nd   = std::numeric_limits<uint32_t>::max();

      for(int d = 0; d < 128 && c - d >= w; ++d)
      {
	uint32_t somma = 0;
	for(int k = -h; k <= h; ++k)
	  for(int l = -w; l <= w; ++l)
	    somma += abs(
		static_cast<int32_t>(left_image.at<uint8_t>(r + k, c + l))
		-
		static_cast<int32_t>(right_image.at<uint8_t>(r + k, c + l - d))
		);
		
	if(somma < min_sum_first)
	{
	  min_sum_2nd = min_sum_first;
	  min_sum_first = somma;
	  disp = d;   //memorizzo disparita' trovata
	}
	else
	{
	  if(somma < min_sum_2nd) 
	  {
	    min_sum_2nd = somma; 
	  }
	}
      }
      if(min_sum_first == min_sum_2nd)
	out.at<float>(r,c) = 0;
      else
...
//
void VDisparity(const cv::Mat & disp, cv::Mat & out, int out_cols = 128)
{
  // immagine di uscita
  //
  // e' una uint16 per essere sicuri di poter accumulare abbastanza valori negli istogrammi
  int dispVal;
  out = cv::Mat(disp.rows, out_cols, CV_16UC1, cv::Scalar(0));
  for(int v = 0; v< disp.rows; v++){
    for(int u =0; u<disp.cols; u++){
      dispVal =  disp.at<float>(v,u);
      out.at<uint16_t>(v, dispVal)++;
    }
  }
...
int main(int argc, char **argv)
{
  ...
  cv::namedWindow("VDisparity", cv::WINDOW_NORMAL);
